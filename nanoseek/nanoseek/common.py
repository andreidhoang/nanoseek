...
def set_reproducibility_seed(seed: int = 42):
    """Full reproducibility seed control across all RNG sources."""
    import random
    import numpy as np
    random.seed(seed)
    np.random.seed(seed)
    ...
    # Note: torch.use_deterministic_algorithms(True) omitted — known slowdown
    logger.info(f'Reproducibility seed set to {seed}')

def compute_init(device_type="cuda", seed: int = 42): # cuda|cpu|mps
