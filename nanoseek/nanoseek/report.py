"""
Utilities for generating NanoSeek training report cards.

Adapted from nanochat/report.py for NanoSeek's simpler workflow:
- Pre-training only (no tokenizer/SFT/RL stages)
...
def get_system_info():
    ...
    info['nanoseek_base_dir'] = os.environ.get('NANOSEEK_BASE_DIR', 'out')
...
def generate_header():
    ...
    header = f"""# NanoSeek training report
...
# the expected files and their order (NanoSeek has simpler workflow)
EXPECTED_FILES = [
    "pre-training.md",
    "base-evaluation.md",
]

# the metrics we're interested in for NanoSeek
BASE_METRICS = ["train_bpb", "val_bpb", "ema_val_bpb", "CORE"]
    ...
    def generate(self):
        ...
        with open(report_file, "w", encoding="utf-8") as out_file:
            ...
            if os.path.exists(header_file):
                ...
                print(f"Warning: {header_file} does not exist. Did you forget to run `nanoseek reset`?")
            ...
            for file_name in EXPECTED_FILES:
                ...
                if file_name == "pre-training.md":
                    final_metrics["training"] = extract(section, ["final_loss", "best_ema_val_bpb", "total_steps", "total_tokens", "total_time_hours"])
                if file_name == "base-evaluation.md":
                    final_metrics["evaluation"] = extract(section, BASE_METRICS)
            ...
            if "training" in final_metrics:
                ...
                for k, v in final_metrics["training"].items():
                    out_file.write(f"- {k}: {v}\n")
                out_file.write("\n")
            
            # Write evaluation metrics
            if "evaluation" in final_metrics:
                out_file.write("### Evaluation\n")
                for k, v in final_metrics["evaluation"].items():
                    out_file.write(f"- {k}: {v}\n")
                out_file.write("\n")
...
# -----------------------------------------------------------------------------
# NanoSeek-specific convenience functions
...
def get_report():
    """Get a Report instance for the current process.
    
    Only rank 0 logs to report (for distributed training).
    Uses NANOSEEK_BASE_DIR env var or defaults to ~/.cache/nanoseek/report
    """
    from .common import get_dist_info
    ddp, ddp_rank, ddp_local_rank, ddp_world_size = get_dist_info()
    if ddp_rank == 0:
        base_dir = os.environ.get("NANOSEEK_BASE_DIR")
        if base_dir:
            report_dir = os.path.join(base_dir, "report")
        else:
            # Use nanoseek subdirectory in base cache
            from .common import get_base_dir as get_nano_base
            report_dir = os.path.join(get_nano_base(), "nanoseek", "report")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate or reset NanoSeek training reports.")
