...
def adamw_step_fused(
    ...
    Parameters:
        β₁ = 0.9       ← momentum decay (window ≈ 10 steps)
        β₂ = 0.999     ← variance decay (window ≈ 1000 steps)
        ε  = 1e-8      ← prevents division by zero
        lr = 1e-3      ← base learning rate
        λ  = 0.01      ← weight decay coefficient

    Algorithm: Initialize: m₀ = 0, v₀ = 0, t = 0
        For each step:
            t ← t + 1
            g ← ∇L(θ)                           ← compute gradient
            θ ← θ × (1 - lr × λ)               ← [5] decoupled weight decay
            m ← β₁·m + (1-β₁)·g                ← [2] first moment (momentum)
            v ← β₂·v + (1-β₂)·g²               ← [3] second moment (adaptive LR)
            m̂ = m / (1 - β₁ᵗ)                   ← [4] bias correction
            v̂ = v / (1 - β₂ᵗ)                   ← [4] bias correction
            θ ← θ - lr × m̂ / (√v̂ + ε)          ← [1+2+3] momentum + adaptive update
    """
    # Weight decay (decoupled, applied before the update)
    p.mul_(1 - lr_t * wd_t) # p = p * (1 - lr_t * wd_t)
    # Update running averages (lerp_ is cleaner and fuses well)
    exp_avg.lerp_(grad, 1 - beta1_t) # exp_avg = exp_avg * (1 - beta1_t) + grad * beta1_t
    exp_avg_sq.lerp_(grad.square(), 1 - beta2_t) # exp_avg_sq = exp_avg_sq * (1 - beta2_t) + grad^2 * beta2_t
    # bias corrections
    bias1 = 1 - beta1_t ** step_t # bias1 = 1 - beta1_t ** step_t
    bias2 = 1 - beta2_t ** step_t # bias2 = 1 - beta2_t ** step_t
...
def muon_step_fused(
    ...
    momentum_buffer.lerp_(stacked_grads, 1 - momentum) # m_t <- m_t-1 * (1 - momentum) + g * momentum
    g = stacked_grads.lerp_(momentum_buffer, momentum) # g <- g * momentum + momentum_buffer * (1 - momentum)

    # Polar express
    X = g.bfloat16()
    X = X / (X.norm(dim=(-2, -1), keepdim=True) * 1.01 + 1e-6) # for numerical stability
    if g.size(-2) > g.size(-1): # Tall matrix (m x n) and (m < n)
        for a, b, c in polar_express_coeffs[:ns_steps]:
            A = X.mT @ X
            B = b * A + c * (A @ A)
            X = a * X + X @ B
    else: # Wide matrix (m x n) and (m > n)
        for a, b, c in polar_express_coeffs[:ns_steps]:
            A = X @ X.mT
            B = b * A + c * (A @ A)
            X = a * X + B @ X
    ...
    v_mean = g.float().square().mean(dim=red_dim, keepdim=True) # v_mean = mean(g^2)
    red_dim_size = g.size(red_dim)
    v_norm_sq = v_mean.sum(dim=(-2, -1), keepdim=True) * red_dim_size
    v_norm = v_norm_sq.sqrt()
    second_momentum_buffer.lerp_(v_mean.to(dtype=second_momentum_buffer.dtype), 1 - beta2)
    step_size = second_momentum_buffer.clamp_min(1e-10).rsqrt() # step_size = 1 / sqrt(second_momentum_buffer)
    scaled_sq_sum = (v_mean * red_dim_size) * step_size.float().square()
    v_norm_new = scaled_sq_sum.sum(dim=(-2, -1), keepdim=True).sqrt() # v_norm_new = sqrt(sum(v_mean * red_dim_size * step_size^2))
    final_scale = step_size * (v_norm / v_norm_new.clamp_min(1e-10)) # final_scale = step_size * (v_norm / v_norm_new)
    g = g * final_scale.to(g.dtype) 

    # Cautious weight decay + parameter update
    lr = lr_t.to(g.dtype)    
    wd = wd_t.to(g.dtype) # wd = weight_decay
    mask = (g * stacked_params) >= 0 # mask = 1 if g * theta >= 0, 0 otherwise
    stacked_params.sub_(lr * g + lr * wd * stacked_params * mask) # theta <- theta - lr * g - lr * wd * theta * mask
...
class DistMuonAdamW(torch.optim.Optimizer):
    """ https://claude.ai/share/ddc04739-5f6f-4d7b-8abf-dd46a5a93b99
    ...
    def _compute_adamw(self, group: dict, info: dict, gather_list: list, rank: int, world_size: int) -> None:
        ...
        After reduce_scatter completes (we wait for it), each GPU runs AdamW only on its shard:
        """
        param_infos = info['param_infos']
        for p in group['params']:
            pinfo = param_infos[p]
            pinfo['future'].wait() # wait for reduce_scatter to complete
            ...
            if pinfo['is_small']:
                ...
                # p_slice is VIEW, not a copy!
                # Modifying p_slice MODIFIES p in-place (same underlying memory).
                # This is why all_gather can later read from p directly.
                if not state:
                    state['step'] = 0
            ...
            if not pinfo['is_small']:
                ...
                ).get_future() # <- launch async all_gather
                gather_list.append(dict(future=future, params=None)) # <- add to gather list
