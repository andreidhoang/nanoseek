"""
Evaluate compression ratio of the NanoSeek tokenizer.

Compares our tokenizer against GPT-2 and GPT-4 tokenizers on various
text types to assess compression efficiency.
...
from scripts.tokenizer import get_tokenizer, RustBPETokenizer
from scripts.dataloader import parquets_iter_batched
...
# Korean text (non-English compression test)
korean_text = r"""
...
def main():
    ...
    # Add training data sample if available
    try:
        train_docs = next(parquets_iter_batched(split="train"))
        train_text = "\n".join(train_docs)
        all_text.append(("fwe-train", train_text))
    except Exception as e:
        print(f"Note: Could not load training data: {e}")
    ...
    for tokenizer_name in ["gpt2", "gpt4", "ours"]:
        ...
        for name, text in all_text:
            ...
            assert decoded == text, f"Roundtrip failed for {tokenizer_name} on {name}"
    ...
    def print_comparison(baseline_name, baseline_results, ours_results, texts):
        ...
        for name, text in texts:
            ...
            # Determine which has better compression
    ...
    try:
        ...
        print(f"\nNote: Could not log to report: {e}")
