...
### Last updated: 2026-03-19 | Phase: Gate 1 Smoke Test PASSING, ready for Phase 3A
...
Phase 3 (STARTING):  Gate 1 smoke test PASSING. Ready for HP grid search.
Phase 4 (NOT STARTED): NanoSeek-1B full training (22B tokens)
Phase 5 (PARTIAL):   Single-stage GRPO exists. 3-stage pipeline not yet built.
```

**What just happened**: Found and fixed 7 critical training bugs (March 18-19).
All previous smoke tests (v1-v4) had broken initialization (zero RMSNorm, zero RoPE).
Smoke test v5 is the FIRST correctly-initialized run. See `docs/TRAINING_BUGS_POSTMORTEM.md`.

**What to do next**: Run Phase 3A anchor HP grid search on RunPod A6000.
See `docs/TRAINING_EXECUTION_PLAN.md` for exact commands.
...
| `docs/TRAINING_EXECUTION_PLAN.md` | Step-by-step training plan: Gate 1 → HP search → 500M → 1B. GPU requirements, cost estimates, RunPod pod specs, pass/fail criteria. |
...
│   │   │   ├── config.py             ← Configs: anchor (480h), 500m (1280h), 1b (2048h)
...
# nano-500M validation (single run, ~14 hrs)
python -m nanoseek.scripts.pre_train \
    --run hp-500m-transfer --scale 500m \
...
cd nanoseek && python -m nanoseek.nanoseek.dataset -n 170  # 170 shards for anchor+500M
```
